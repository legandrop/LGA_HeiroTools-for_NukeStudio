...
def print_clip_properties(clip):
    ...
    try:
        ...
        if source_item:
            ...
            debug_print(
                f"Media File Path: {source_item.mediaSource().fileinfos()[0].filename()}"
            )
...
class TrimInToPlayhead:
    ...
    def setup_actions(self):
        ...
        hiero.core.events.registerInterest(
            "kShowContextMenu/kTimeline", self.timeline_context_menu
        )
            ...
            for item in items:
                if item != current_item:  # No comparar con el mismo clip
                    ...
                    if item.timelineIn() < new_in and new_in <= item.timelineOut():
                        debug_print(
                            f"Superposición detectada con clip: {item.name()} ({item.timelineIn()}-{item.timelineOut()})"
                        )
                        return True
            return False
        except Exception as e:
    ...
    def get_min_possible_timeline_in(self, clip):
        ...
        try:
            ...
            available_source_frames_before = (
                source_in - 0
            )  # source_in actual menos el mínimo posible (0)
            ...
            debug_print(
                f"  - Available Source Frames Before: {available_source_frames_before}"
            )
            ...
            if current_index > 0:
                ...
                debug_print(
                    f"  - Clip anterior: {prev_clip.name()} termina en frame {min_in_prev_clip}"
                )
            ...
            if min_in_prev_clip is not None:
                ...
                debug_print(
                    f"  - Límite final (mayor entre media y clip anterior): {min_in}"
                )
            else:
                min_in = min_in_media
    ...
    def extend_clip_to_minimum(self, clip, target_frame):
        ...
        try:
            ...
            if new_source_in < 0:
                debug_print(
                    f"El nuevo source in sería negativo ({new_source_in}). Ajustando a 0."
                )
                new_source_in = 0
            ...
            debug_print(
                f"  - Timeline IN: {timeline_in} -> {target_frame} (diferencia: {frame_difference})"
            )
            debug_print(f"  - Source IN: {source_in} -> {new_source_in}")
            debug_print(
                f"  - Nota: El playhead estaba antes del clip. Se ha extendido al mínimo posible."
            )
            ...
            debug_print(
                f"  - Target frame (tipo: {type(target_frame)}): {target_frame}"
            )
            return False
    ...
    def trim_material_to_playhead(self, clip, playhead_frame):
        ...
        try:
            ...
            if playhead_frame >= timeline_out:
                debug_print(
                    f"El playhead ({playhead_frame}) está después del final del clip {clip.name()} ({timeline_out})"
                )
                return False
            ...
            if new_source_in >= source_out:
                debug_print(
                    f"El recorte excedería los límites del source. No se puede recortar."
                )
                return False
            ...
            try:
                ...
                debug_print(
                    f"Error durante el recorte: {e}. Intentando revertir cambios..."
                )
                try:
            ...
            debug_print(
                f"  - Timeline IN: {timeline_in} -> {new_timeline_in} (recortado {frames_to_trim} frames)"
            )
            debug_print(
                f"  - Source IN: {source_in} -> {new_source_in} (recortado {frames_to_trim} frames)"
            )
        ...
        for clip in selected_clips:
            try:
                ...
                if playhead_frame < timeline_in:
                    debug_print(
                        f"El playhead ({playhead_frame}) está antes del inicio del clip {clip.name()} ({timeline_in})"
                    )
                    ...
                    if playhead_frame < min_in:
                        ...
                        debug_print(
                            f"El playhead ({playhead_frame}) está antes del límite mínimo ({min_in}). Se ajustará al mínimo."
                        )

                    # Verificamos si hay superposición con otro clip en el frame objetivo
                    parent_track = clip.parent()
                    if self.check_for_overlapping_items(
                        parent_track, clip, target_frame
                    ):
                        debug_print(
                            f"No se puede extender: causaría superposición con otro clip"
                        )
                        continue
