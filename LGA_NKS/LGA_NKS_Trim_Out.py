...
class TrimOutToPlayhead:
    ...
    def get_max_possible_timeline_out(self, clip):
        ...
        try:
            ...
            if current_index < len(sorted_items) - 1:
                ...
                debug_print(
                    f"  - Siguiente clip: {next_clip.name()} en frame {max_out_next_clip}"
                )
            ...
            if max_out_next_clip is not None:
                ...
                debug_print(
                    f"  - Límite final (menor entre media y siguiente clip): {max_out}"
                )
            else:
    ...
    def adjust_clip_out(self, clip, target_frame, is_max_limit=False):
        ...
        try:
            ...
            debug_print(
                f"  - Timeline OUT: {timeline_out} -> {target_frame} (diferencia: {frame_difference})"
            )
            debug_print(f"  - Source OUT: {source_out} -> {new_source_out}")

            if is_max_limit:
                debug_print(
                    f"Nota: El playhead excedía el límite máximo. Se ha ajustado al máximo posible."
                )
    ...
    def trim_out_to_playhead(self):
        ...
        for clip in selected_clips:
            try:
                ...
                debug_print(
                    f"  - Playhead offset desde inicio del clip: {playhead_offset_from_clip_start}"
                )
                ...
                if max_out and playhead_frame > max_out:
                    ...
                    is_max_limit = True
                    debug_print(
                        f"El playhead ({playhead_frame}) excede el límite máximo ({max_out}). Se ajustará al máximo."
                    )
                ...
                if self.check_for_overlapping_items(parent_track, clip, target_frame):
                    debug_print(
                        f"No se puede recortar: causaría superposición con otro clip"
                    )
                    continue
