...
- Update automatico: proyectos abiertos muestran boton `Update` cuando existe version mas nueva en disco. La version mas alta se busca SOLO entre archivos del mismo proyecto: `encontrar_version_mas_alta()` filtra por `obtener_clave_proyecto_archivo()`, la misma clave con la que agrupa el escaneo. No usar globs por prefijo aca — `ERSO*` tambien matchea `ERSO_BREAKDOWN_*` y termina ofreciendo la version de otro proyecto.
