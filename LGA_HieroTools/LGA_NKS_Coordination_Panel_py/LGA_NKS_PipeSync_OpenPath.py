...
____________________________________________________________________
  ...
  LGA_NKS_PipeSync_OpenPath v1.00 | Lega

  Abre la carpeta del shot seleccionado en PipeSync usando CLI (--path).
...
def setup_debug_logging(script_name="PipeSync_OpenPath"):
...
debug_logger = setup_debug_logging(script_name="PipeSync_OpenPath")
...
def get_shot_path(file_path):
    ...
    # 1) Detección por patrón de nombre de shot (ej: PROJF_050_010)
    ...
    # 2) Detección por estructura de ruta (root + proyecto/grupo/shot)
    root_len = 0
...
def build_pipesync_cmd(shot_path):
    if sys.platform == "darwin":
        debug_print("PipeSync CLI no está disponible en macOS todavía.")
        return None

    if Desarrollo:
        dev_exe = r"C:\Portable\LGA_PipeSync_2\build\PipeSync.exe"
        if os.path.exists(dev_exe):
            pipesync_exe = dev_exe
            debug_print("Usando versión de desarrollo de PipeSync")
        else:
            pipesync_exe = r"C:\Portable\LGA\PipeSync\PipeSync.exe"
            debug_print("Versión de desarrollo no encontrada, usando versión portable")
    else:
        pipesync_exe = r"C:\Portable\LGA\PipeSync\PipeSync.exe"

    return [pipesync_exe, "--path", shot_path]


def main():
    """Función principal que abre PipeSync con la ruta del shot seleccionado"""
    debug_print("=== PIPESYNC OPEN PATH ===")
    ...
    try:
        ...
        cmd = build_pipesync_cmd(shot_path)
        if not cmd:
            debug_print("No se pudo construir el comando para PipeSync")
            return
        ...
        try:
            ...
            debug_print("PipeSync abierto correctamente")
        except Exception as cmd_error:
            debug_print(f"Error al ejecutar PipeSync: {cmd_error}")
