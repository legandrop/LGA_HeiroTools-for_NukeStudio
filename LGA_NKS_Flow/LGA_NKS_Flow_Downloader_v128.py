...
4b- en Terminal (mac): (se usa : en lugar de ; para separar rutas)
    cd /Users/leg4/.nuke/Python/Startup/LGA_NKS_Flow/Compilar
    pyinstaller --onefile --add-data "Data/no_icon.icns:Data" --add-data "Data/LGA.icns:Data" --add-data "Data/settings_off.png:Data" --add-data "Data/settings_on.png:Data" --add-data "Data/CTkScrollableDropdown/ctk_scrollable_dropdown.py:Data/CTkScrollableDropdown" --add-data "Data/LGA_NKS_Flow_Downloader_CCTK_Theme.json:Data" -i "Data/LGA.icns" LGA_NKS_Flow_Downloader.py
...
debug_test_offline = False  # Cambiar a True para probar sin conexion a ShotGrid
...
class FlowDataDownloader:
    ...
    def find_tasks_for_shot(self, shot_id):
        ...
        fields = ['id', 'content', 'sg_description', 'sg_status_list', 'task_assignees']
        try:
                ...
                for shot in shots:
                    ...
                    for task in tasks:
                        if 'Comp' in task['content']:
                            ...
                            task_data = {
                                "task_type": "Comp",
                                "task_description": task.get('sg_description', 'Sin descripcion'),
...
class JsonComparisonManager:
    ...
    def new_versions_window(self, versions):
        ...
        if platform.system() == "Windows":
            ...
            dialog.after(200, lambda: dialog.iconbitmap(icon_path)) # Reestablecer el icono, Solucion para el problema en Windows con customtkinter
        elif platform.system() == "Darwin":
            icon_path = resource_path("LGA.ico")
            dialog.resizable(False, False)  # Esto podria ayudar a evitar que se abran como pestanas
            setup_window_icon(dialog, icon_path)        
    ...
    def uploaded_versions_window(self, versions):
        ...
        if platform.system() == "Windows":
            ...
            icon_path = resource_path("LGA.ico")
            details_window.resizable(False, False)  # Esto podria ayudar a evitar que se abran como pestanas
            setup_window_icon(details_window, icon_path)
...
class MainApp:
    ...
    # Metodo nuevo de la version para mac:
    ...
    # Metodo cambiado por la version para mac. Cambia el dropdown de tiempos:
    def create_widgets(self):
        ...
        if platform.system() == "Windows":
            ...
            self.interval_option = tk.StringVar(value=self.sync_interval)
            self.interval_option_widget = ctk.CTkOptionMenu(control_frame, variable=self.interval_option, values=values, command=self.on_interval_change, width=85)
        ...
        version_label = ctk.CTkLabel(bottom_frame, text="v1.5", anchor="se")
        version_label.pack(side=ctk.RIGHT, padx=30)
        self.setup_tooltip(version_label, "Lega | 2024")
    ...
    def toggle_sync(self):
        if self.download_thread and self.download_thread.is_alive():
            ...
            self.sg_downloader = FlowDataDownloader(self.url, self.script_name, self.api_key)
    ...
    def settings_window(self):
       ### Ventana que muestra cuantas versiones nuevas hay
        ...
        if platform.system() == "Windows":
            ...
            self.settingsW.after(200, lambda: self.settingsW.iconbitmap(icon_path))  # Reestablecer el icono. Solucion para el problema en Windows con customtkinter
        elif platform.system() == "Darwin":
            icon_path = resource_path("LGA.ico")
    ...
    # Metodo cambiado por la version para mac:
    def download_and_sync(self):
        ...
        try:
            if not debug_test_offline:
                # Crear instancia de FlowDataDownloader
                self.sg_downloader = FlowDataDownloader(self.url, self.script_name, self.api_key)
                ...
                if self.sg_downloader.cancel_download:
                    # Si la descarga fue cancelada, detener la sincronizacion y reanudar el contador
                    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    print(f"{current_time} - Sync cancelado")
                    self.sync_now_button.configure(text="Sync Now", command=self.sync_now)
                    self.update_countdown()
                    return

            # Proceder con la sincronizacion despues de la descarga o si esta en modo offline
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            ...
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            print(f"{current_time} - No se puede conectar a ShotGrid. Verifique su conexion a Internet.")
            print(f"{current_time} - download_and_sync error: {e}")
            logging.error(f"Error in download_and_sync: {e}", exc_info=True)
            self.sync_now_button.configure(text="Sync Now", command=self.sync_now)
            self.update_countdown()

        finally:
            self.sg_downloader = None  # Resetear la instancia de FlowDataDownloader
    ...
    # Tal vez haya que cambiarlo en la version para mac:
    ...
    def setup_tooltip(self, widget, text):
        # Funcion para mostrar el tooltip Lega 2024
        def on_enter(event):
            x = widget.winfo_rootx() + widget.winfo_width() - 100
            ...
            if platform.system() == "Windows":
                # Aplicar transparencia para Windows
                self.tooltip_window.attributes('-transparentcolor', '#26272a')
            else:
                # Aplicar transparencia en macOS o Linux
    ...
    def stop_sync(self):
        ...
        # Cancelar la proxima sincronizacion programada (esta parte es nueva de MAC)
        if self.sync_after_id:
            self.root.after_cancel(self.sync_after_id)
            self.sync_after_id = None
    ...
    def initialize_sg_manager(self):
        ...
        if not debug_test_offline: # Si no esta en modo offline, asegurarse de que se ha descargado el archivo
        ...
        # Siempre inicializa el JsonLocalManager, lo que implica volver a leer el JSON
        self.sg_manager = JsonLocalManager(json_file_path)
        self.hiero_ops = JsonComparisonManager(self.root, self.sg_manager)
...
def setup_window_icon(window, icon_path):
    try:
        if platform.system() == "Windows":
            ...
            icon = Image.open(icon_path.replace(".ico", ".icns"))
            window.iconphoto(True, ImageTk.PhotoImage(icon))
        else:
            print("Unsupported OS for setting window icon.")
    except Exception as e:
...
def download_project_data(sg_downloader, project_names, json_SG_output_file):
...
def setup_logging(config):
    log_file_directory = config.get('log_file_path')

    # Verifica si la ruta de config.json existe y es valida
    if log_file_directory and os.path.exists(log_file_directory):
        debug_print(f"Usando ruta de config.json: {log_file_directory}")
    else:
        print(f"Ruta en config.json no valida o inexistente: {log_file_directory}")
        # Si no es valida, usar la ruta del script y apuntar a la subcarpeta 'Data'
        current_script_path = get_script_path()
        log_file_directory = os.path.join(current_script_path, 'Data')
        print(f"Usando ruta del script: {log_file_directory}")
    ...
    # Verifica si el archivo .log existe, si no, lo creara
    if not os.path.isfile(log_file_path):
        print(f"El archivo .log no existe. Creandolo en la ruta: {log_file_path}")
        with open(log_file_path, 'w', encoding='utf-8') as f:
            f.write('')  # Crear un archivo vacio

    debug_print(f"Log File Path: {log_file_path}")
...
def main():
    ...
    app = MainApp(root, sg_url, sg_script_name, sg_api_key)
    root.mainloop()
