...
class ShotGridManager:
    ...
    def attach_images_to_note(self, note_id, version_id, image_paths):
        ...
        Renombra los archivos temporalmente para incluir el numero de frame.
        """
        if not self.sg:
            debug_print("ShotGrid no inicializado")
            return False
        ...
        try:
            ...
            for image_path in image_paths:
                ...
                # Crear nombre de archivo con convencion de ShotGrid
                ...
                # Subir archivo a ShotGrid como attachment
                try:
                    # Crear un File entity temporal para poder adjuntarlo
                    file_data = {
                        "code": new_filename,
                        "project": {
                            "type": "Project",
                            "id": self.get_project_id_from_version(version_id),
                        },
                    }

                    # Crear el entity primero
                    attachment_entity = self.sg.create("Attachment", file_data)

                    # Subir el archivo al entity creado
                    uploaded_file = self.sg.upload(
                        "Attachment",
                        attachment_entity["id"],
                        temp_file_path,
                        field_name="this_file",
                    )

                    if uploaded_file and attachment_entity:
                        # Vincular el archivo a la nota
                        self.sg.update(
                            "Note",
                            note_id,
                            {
                                "attachments": [
                                    {
                                        "type": "Attachment",
                                        "id": attachment_entity["id"],
                                    }
                                ]
                            },
                        )
                        attached_files.append(attachment_entity)
                        debug_print(
                            f"Imagen adjuntada exitosamente: {new_filename} (ID: {attachment_entity['id']})"
                        )
            ...
            debug_print(
                f"Adjuntadas {len(attached_files)} imagenes de {len(image_paths)} totales"
            )
            return len(attached_files) > 0
...
class Worker(QRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            # Borrar imagenes si se completó exitosamente y el usuario lo solicitó
            if self.should_delete_images:
                debug_print("Borrando carpeta ReviewPic_Cache como solicitó el usuario")
